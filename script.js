...
function isBlocked(t){
  ...
  // A tile is blocked only when a still-visible tile above it actually
  // overlaps its clickable area. The highest visible tile is always playable.
  return board.some(o=>o.id!==t.id && !o.removed && o.z>t.z && overlaps(t,o));
}
function renderBoard(){const el=$("#board");el.querySelectorAll(".tile").forEach(x=>x.remove());board.filter(t=>!t.removed).forEach(t=>{const b=document.createElement("button");b.className="tile";b.dataset.id=t.id;b.style.left=t.x+"px";b.style.top=t.y+"px";b.style.setProperty("--z",t.z);b.style.zIndex=t.z; b.style.setProperty("--dx",((t.id*17)%7-3)+"px");if(isBlocked(t))b.classList.add("blocked");else { b.setAttribute("aria-label","Select "+t.icon); b.addEventListener("click",()=>pick(t.id)); }if(t.special)b.classList.add("special-"+t.special);b.textContent=t.icon;el.appendChild(b)})}
...
function useShuffle(){if(!state.pu.shuffle){buyPowerup('shuffle',100);return}state.pu.shuffle--;const alive=board.filter(t=>!t.removed),positions=alive.map(t=>({x:t.x,y:t.y}));shuffle(positions);alive.forEach((t,i)=>{t.x=positions[i].x;t.y=positions[i].y});save();renderBoard();updatePowerups();playSound('click');vibrate([18]);showToast('BOARD SHUFFLED','good')}
function useHint(){
  if(gameOver||pickBusy)return;
...
function buyPowerup(type,price){
  ...
  playSound('buy');
  vibrate([16,25]);
  showToast(`${type.toUpperCase()} +1`,'buy');
  // The shop is a live modal, so refresh it immediately. Previously the
  // underlying state changed but the open shop still displayed the old coin
  // balance, making purchases look like they had not happened.
  if(!$('#modal').classList.contains('hidden')) openShop();
}
...
function init(){
  ...
  $("#board").addEventListener("click",e=>{
    ...
    if(tile && !tile.classList.contains("blocked")) pick(Number(tile.dataset.id));
  });
